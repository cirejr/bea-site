...
export function BeaInstitutionalCtaSection(_props: BeaInstitutionalCtaSectionProps) {
  const t = useTranslations("homeCta")

  return (
    <section className="relative overflow-hidden bg-bea-on-primary-fixed-variant py-24">
      <div className="absolute inset-0 opacity-10">
        <div className="bea-arrow-watermark h-full w-full" />
      </div>
      <div className="relative z-10 mx-auto max-w-7xl px-8 text-center">
        ...
        <p className="mx-auto mb-12 max-w-2xl font-bea-body text-lg italic text-bea-primary-fixed">
          {t("description")}
        </p>
        <div className="flex flex-col justify-center gap-6 sm:flex-row">
          <Link href="/contact">
            <Button
              className="h-auto rounded-full bg-white px-10 py-4 font-bea-headline text-base font-extrabold text-bea-primary shadow-xl hover:scale-105 hover:bg-white"
            >
              {t("ctaPrimary")}
            </Button>
          </Link>
